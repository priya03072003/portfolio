...
const Projects = () => {
  ...
  const categories = ['All', 'Frontend', 'Backend', 'Fullstack', 'DevOps', 'AI'];

  const projects = [
    {
      id: 1,
      title: "Donut UI - E-commerce",
      category: "Frontend",
      desc: "Innovative visual design prototype showcasing dynamic layouts, custom animations, and typography.",
    ...
    {
      id: 2,
      title: "RoadMaster Portal",
      category: "Fullstack",
      desc: "Comprehensive interface for managing and purchasing insurance policies securely.",
      link: "#",
      github: "#",
      tags: ["React", "Express", "PostgreSQL", "Tailwind"],
      media: [
        { type: 'image', url: 'https://images.unsplash.com/photo-1454165804606-c3d57bc86b40?w=800&q=80' },
          ...
          <div className="flex flex-wrap items-center justify-start md:justify-center gap-4 bg-[var(--surface-color)] p-2 rounded-[2rem] border border-[var(--border-color)] w-max mx-auto overflow-hidden shadow-sm">
            {categories.map((cat) => (
              <button
                ...
                className="relative px-6 py-2 text-sm font-bold rounded-full transition-colors cursor-none"
                style={{ color: activeFilter === cat ? 'white' : 'var(--text-secondary)' }}
              >
                {activeFilter === cat && (
                  <motion.div
